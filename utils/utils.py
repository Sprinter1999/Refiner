    ...
    if 'RS' in dataset:
        if dataset_form == "imagefolder":
            ...
            _, y_train, _, _, net_dataidx_map, traindata_cls_counts = load_parquet_data(datadir, n_parties=n_parties,
                                                                                        ...
                                                                                        train_distribution=partition,
                                                                                        val_distribution=val_distribution)
    return (net_dataidx_map, traindata_cls_counts, y_train)
...
def compute_accuracy(model, dataloader, get_confusion_matrix=False, device="cpu", multiloader=False):
    ...
    if multiloader:
        ...
        with torch.no_grad():
            for batch_idx, (x, target, ind) in enumerate(dataloader):
                if device != 'cpu':
                    x, target = x.cuda(), target.to(dtype=torch.int64).cuda()
                _, _, out = model(x)
                loss = criterion(out, target)
    ...
    if was_training:
        model.train()
    if get_confusion_matrix:
        return acc, f1, precision, recall, conf_matrix, avg_loss
    return acc, f1, precision, recall, avg_loss


def get_train_dataloader(args, client_idx, test_bs=32, custom_labels=None):
...
def get_test_dataloader(args, test_bs=256):
...
def flip_labels(y, noise_rate, noise_pattern, n_classes, seed=None):
    """
    y: numpy array, shape (N,)
    noise_rate: float, 噪声比例
    noise_pattern: 'symmetric' or 'pairflip'
    n_classes: int, 类别数
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    for idx in noisy_idx:
        ...
        if noise_pattern == 'symmetric':
            new_label = np.random.choice([l for l in range(n_classes) if l != old_label])
        elif noise_pattern == 'pairflip':
            new_label = (old_label + 1) % n_classes
        else:
            raise ValueError("Unknown noise pattern")
