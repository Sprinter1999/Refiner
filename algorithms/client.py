...
import torchvision
import torchvision.transforms as transforms  # Added to support data augmentation
from utils.utils import compute_accuracy


from algorithms.symmetricCE import SCELoss
from algorithms.fedrefiner import train_net_fedrefiner

def train_net(net_id, net, global_model,train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu", logger=None):
    net.cuda()
    ...
    for epoch in range(epochs): 
        ...
        for batch_idx, (x, target, idx) in enumerate(train_dataloader):
            ...
            num_classes = net.l3.out_features 
            invalid_labels = target[(target < 0) | (target >= num_classes)]
...
def train_net_scaffold(net_id, net, global_model, c_local, c_global, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu", logger=None):
    ...
    return None, c_delta_para

def train_net_symmetricCE(net_id, net, global_model, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu", logger=None):
    # logger.info('Training network %s' % str(net_id))
    net.cuda()
    if args_optimizer == 'adam':
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=lr, weight_decay=args.reg)
    ...
    criterion = SCELoss(alpha=args.sce_alpha, beta=args.sce_beta, num_classes=net.l3.out_features)
...
def train_net_moon(net_id, net, global_net, previous_nets, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, temperature, args,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, target, idx) in enumerate(train_dataloader):
            ...
            if pro1.dim() == 1:
                pro1 = pro1.unsqueeze(0)
            if pro2.dim() == 1:
                pro2 = pro2.unsqueeze(0)
            if out.dim() == 1:
                out = out.unsqueeze(0)
...
def local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, round=None, device="cpu", logger=None):
    ...
    for net_id, net in nets.items():
        ...
        n_epoch = args.epochs
        if args.alg == 'fedavg' or args.alg == 'fednova' or args.alg=='feddisco' or args.alg=='fedavgm' or args.alg=='fedavg-dp':
            testacc = train_net(net_id, net, global_model,train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args,
            ...
            testacc = train_net_symmetricCE(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
                                                  args.optimizer, args, device=device, logger=logger)
        
        elif args.alg == 'fedrefiner':
            # FedRefiner有自己的主算法函数，这里不需要单独处理
            testacc = train_net_symmetricCE(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
                                                  args.optimizer, args, device=device, logger=logger)
            ...
            testacc = train_net_moon(net_id, net, global_model, prev_models, train_dl_local, test_dl, n_epoch, args.lr,
